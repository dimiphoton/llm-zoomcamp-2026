...
```bash
# Sur ce poste, uv echoue (certificat TLS du proxy) -> utiliser pip (conda)
pip install gitsource minsearch openai toyaikit python-dotenv
```

Créer un `.env` (à la racine du dépôt ou à côté du notebook) :
...
## Rejouer le notebook en ligne de commande

```bash
python -m jupyter nbconvert --to notebook --execute --inplace homework1.ipynb
```
